...
describe('PromptService', () => {
  ...
  describe('registerTemplate', () => {
    ...
    it('should reject templates without an ID', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Template must have an ID and template text');
    });
  ...
  describe('fillTemplate', () => {
    ...
    it('should return null for non-existent templates', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Template with ID non-existent not found');
    });
    ...
    it('should warn about unreplaced parameters', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Some parameters were not replaced: {{test}}');
    });
  ...
  describe('generateContent', () => {
    ...
    it('should use streaming when enabled', async () => {
      ...
      vi.mocked(mockAIClient.generateContentStream).mockImplementation(
        async (_prompt: string, callback?: (chunk: string, progress: number) => void) => {
          if (callback) {
            callback('Chunk 1', 25);
            callback('Chunk 2', 50);
            callback('Chunk 3', 75);
