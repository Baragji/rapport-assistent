...
export const useFormValidation = (
    ...
    (field: string, value: unknown, timing: ValidationTiming = 'immediate'): ValidationResult => {
      ...
      if (customRules?.[field]?.dependsOn) {
        ...
        const dependenciesValid = dependencies.every(dep => 
          errors[dep]?.isValid || !touched[dep]
        );
      ...
      if (field === 'content') {
        ...
        if (customRules?.content?.maxLength) {
          return validateMaxLength(value as string, customRules.content.maxLength, 'Report content');
        }
  ...
  const validateForm = useCallback(() => {
    ...
    const areCustomFieldsValid = customRules 
      ? Object.keys(customRules)
          ...
          .every(field => newErrors[field]?.isValid)
      : true;
  ...
  }, [options.validateOnMount, options.validateOnInit]);
  ...
  }, [formData, options.validateOnChange]);
